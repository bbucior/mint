...
/**
 * Define pair potential settings from text.
 * @param input [in] Text containing 
 */
...
/**
 * Set Buckingham function from text input
 */
...
/**
 * Set Power function from text input
 * @param input [in] Text describing desired potential
 */
...
void HardSphere::set(const Text& input) {
	// No data
	if (!input.length())
		return;
	...
	// Output
	Output::newline();
	Output::print("Hard-sphere between ");
	Output::print(_element1.symbol());
	Output::print(" and ");
	Output::print(_element2.symbol());
	Output::increase();
